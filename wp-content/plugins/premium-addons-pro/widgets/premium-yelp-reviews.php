     ...
     protected function _register_controls() {
        ...
        $this->add_control('api_key',
            [
                ...
                'label_block'   => true,
                'default'       => 'lXqtwdDAbShfuikjtcPTbrGW1jnLPWhnW6zAFhsAJfOnYsJr2cI5Yd-0Q9Zq9AMO1i93plURrhDNZNyHpsrjbKqoS7RrLKT4F8S-4CUl-SWml8DoXXmwDzmjQbAYXXYx',
        ...
        $this->add_control('words_num',
            [
                'label'         => __( 'Review Text Length', 'premium-addons-pro' ),
                'type'          => Controls_Manager::NUMBER,
                'min'           => 1,
                'default'       => 20
            ]
...
//            $this->add_render_attribute('place_dir', 'class', $settings['place_dir'] );
//        }
        
        $this->add_render_attribute( 'reviews', 'class', 'premium-fb-rev-content' );
        
//        if( 'inline' === $settings['reviews_display'] ) {
//            $this->add_render_attribute( 'dir', 'class', $settings['reviews_dir'] );
//        }
        ...
        $place                  = $response_results_place;
       
        if ( isset( $response_results->reviews ) ) {

            $reviews = $response_results->reviews;

            update_option('premium_yelp_reviews-' . $this->get_id() , $reviews );
            update_option('premium_yelp_reviews-place-' . $this->get_id() , $place_id );

        } elseif ( ! isset( $response_results->reviews ) ) {
            
            $stored_id = get_option( 'premium_yelp_reviews-place-' . $this->get_id() );
            
            if( !empty( $stored_id ) && $stored_id == $place_id ) {
                $reviews = get_option('premium_yelp_reviews-' . $this->get_id() , array() );
            }
            
        } elseif ( empty( $api_key ) || empty ( $place_id ) ) { ?>
            <div class="premium-fbrev-error">
                <?php echo __('Please Enter a Valid API Key & Business ID','premium-addons-pro'); ?>
            </div>
        <?php
        }
    ...
    <div class="premium-fb-rev-container yelp-reviews <?php echo 'premium-reviews-'. $settings['reviews_style']; ?>" data-col="<?php echo esc_attr( $reviews_number ); ?>" data-style="<?php echo esc_attr($settings['reviews_style']); ?>" data-carousel="<?php echo $carousel; ?>" data-play="<?php echo $play; ?>" data-speed="<?php echo $speed; ?>" data-rtl="<?php echo $rtl; ?>">
