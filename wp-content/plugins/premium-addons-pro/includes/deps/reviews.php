...
function premium_fb_rev_api_rating( $page_id, $page_access_token ) { 
    ...
    $api_url = PREMIUM_FB_REV_GRAPH_API . $page_id . "/ratings?access_token=" . $page_access_token . "&fields=reviewer{id,name,picture.width(200).height(200)},created_time,rating,recommendation_type,review_text,open_graph_story{id}";
...
function premium_fb_rev_trim_text( $text, $size ) {
    ...
    if ( 0 < $size && str_word_count( $text ) >= $size ) {
